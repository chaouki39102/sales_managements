...
protected function afterUpdate(Model $item, array $data, $request): void
{
    ...
    private function createDocumentLines(CommercialDocument $document, array $lines): void
    {
        ...
        if (!empty($productIds)) {
            $this->assertProductsBelongToCompany(
    ...
    private function createStockMovements(CommercialDocument $document): void
    {
        ...
        // ✅ جلب نوع الحركة من DB بـ direction + company_id — لا IDs ثابتة
        //    لأن كل شركة جديدة تحصل على IDs مختلفة بعد الـ seeding
        //    direction: +1 = إدخال (شراء/استلام)، -1 = إخراج (بيع/تسليم)
        $expectedDirection   = $direction > 0 ? 1 : -1;
        $stockMovementTypeId = DB::table('stock_movement_types')
            ->where('company_id', $document->company_id)
            ->where('direction', $expectedDirection)
            ->value('id');

        if (!$stockMovementTypeId) {
            Log::error("createStockMovements: لم يُعثر على stock_movement_type بـ direction={$expectedDirection} للشركة#{$document->company_id}");
            return;
        }
    ...
    private function generateDocumentNumber(DocumentType $documentType, int $companyId): string
{
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // GUARD: التحقق من انتماء منتجات الأسطر للشركة الحالية
    //
    // ✅ تُستخدم إذا لم يُعرِّف الـ ValidatesTenantRelations trait
    //    دالة validateTenantRelationsMany() — طبقة أمان إضافية
    //    تمنع حقن product_ids من شركات أخرى عبر الـ API
    // ═══════════════════════════════════════════════════════════════════════

    /**
     * التحقق من أن جميع product_ids المُرسَلة تنتمي للشركة الحالية.
     *
     * ✅ تعمل دائماً — لا تعتمد على وجود validateTenantRelationsMany في الـ trait
     * ✅ تمنع حقن IDs من شركات أخرى عبر الـ API (multi-tenant security)
     *
     * @param int[] $productIds
     * @throws BusinessRuleException إذا وُجد product_id خارج نطاق الشركة
     */
    private function assertProductsBelongToCompany(array $productIds, int $companyId): void
    {
        ...
        if ($found !== count($productIds)) {
            throw new BusinessRuleException(
                'بعض المنتجات في الأسطر لا تنتمي لشركتك أو غير موجودة.',
                422
            );
        }
    }
