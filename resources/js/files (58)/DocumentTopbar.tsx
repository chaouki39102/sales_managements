...
/**
 * ═══════════════════════════════════════════════════════════════════════
 * 🔧 إصلاح: يُغلق قائمة منسدلة عند الضغط خارجها أو عند Escape.
 * بديل React-safe للنمط القديم في DocumentFooter الذي كان يستخدم
 * document.getElementById('export-menu') مباشرة (تلاعب DOM خارج React،
 * بلا إغلاق عند الضغط خارج القائمة، وبلا دعم Escape).
 * ═══════════════════════════════════════════════════════════════════════
 */
...
/**
 * ═══════════════════════════════════════════════════════════════════════
 * DocumentTopbar — يستبدل DocumentHeaderSection + شريط الإجراءات في
 * DocumentFooter معاً. في نمط الصفحة الكاملة (بدل الـ Modal)، الإجراءات
 * الأساسية (حفظ/إلغاء/طباعة/تصدير) تبقى ظاهرة دوماً أعلى الصفحة بدل أن
 * تنتظر المستخدم في أسفل عمود تمرير طويل — تماماً كما في Odoo/SAP.
 *
 * إصلاحات مدمجة هنا:
 * - شارة حالة واحدة فقط (statusPill) بأولوية واضحة بدل تكديس Badges
 *   متعددة (ملغى/مقفل/معتمد) كانت تتزاحم أفقياً في النسخة القديمة.
 * - queries حذف/مرتجع مجمّعة في قائمة "المزيد" بدل أزرار منفصلة تُزاحم
 *   زر الحفظ الأساسي.
 * - تعطيل زر "إلغاء/إغلاق" وزر "رجوع" متطابق تماماً (نفس canDismiss)
 *   بدل التضارب السابق بين الهيدر (isPending فقط) والفوتر
 *   (isPending || successMsg).
 * - دعم Escape للرجوع (متوافق مع نفس شرط التعطيل).
 * ═══════════════════════════════════════════════════════════════════════
 */
export default function DocumentTopbar({
  ...
  return (
    <div style={{
      ...
      {/* ─── الإجراءات — دائماً ظاهرة أعلى الصفحة، لا تنتظر التمرير للأسفل ─── */}
      <div style={{ marginRight: 'auto', display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
        ...
        {/* تصدير — Dropdown مبني على state، يُغلق عند الضغط خارجه أو Escape */}
        ...
        {/* المزيد — حذف/مرتجع مجمّعة بدل أزرار منفصلة تُزاحم الحفظ */}
        {hasMoreItems && (
