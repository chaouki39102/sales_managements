// resources/js/pages/pos/POSKioskPage.tsx
// ════════════════════════════════════════════════════════════════════════════
// إصلاحات عن النسخة السابقة:
//   1. pos.incrementSession() أُزيلت — استُبدلت بـ incrementMut.mutate()
//   2. party_id: null ثابت أُزيل — يُمرَّر من pos.client إن وُجد
//   3. delivery_type: undefined أُزيلت (حقل غير موجود في DocumentCreateInput)
//   4. client_id أُصلح → party_id
//   5. OpenSessionModal مُضاف (الكشك يحتاج جلسة أيضاً)
//   6. printThermal تستقبل بيانات الشركة الحقيقية
// ════════════════════════════════════════════════════════════════════════════
  ...
  const handleCompleteSale = async (params: {
    ...
    try {
      ...
      const res = await documentsApi.create({
        ...
        // ✅ party_id بدلاً من client_id (حقل غير موجود)
        party_id:         pos.client?.id ?? null,
        warehouse_id:     defaultWarehouse.id,
        fiscal_year_id:   fiscalYearId,
        document_date:    new Date().toISOString().slice(0, 10),
        notes:            null,
        // ✅ delivery_type أُزيلت (حقل غير موجود في DocumentCreateInput)
