...
export default function ProductsReportPage() {
  ...
  const handleExport = async () => {
    if (!data) return;
    const docsHeaders = ['#', 'المنتج', 'المرجع', 'العائلة', 'كمية المشتريات', 'قيمة المشتريات', 'كمية المباع', 'قيمة المبيعات', 'المخزون الحالي', 'الحالة'];
    const costHeaders = ['متوسط سعر الشراء', 'تكلفة البضاعة', 'الربح المقدر', 'الهامش %'];
    const headers = canViewCost ? [...docsHeaders.slice(0, 6), ...costHeaders, ...docsHeaders.slice(6)] : docsHeaders;
    const rows = data.products.map((r, i) => {
      const row = [
        i + 1,
        r.name,
        r.ref,
        ...
        r.sales_ht,
        r.stock_quantity,
        STATUS_META[r.stock_status]?.label ?? r.stock_status,
      ] as (string | number)[];
      if (canViewCost) {
        row.splice(6, 0, r.avg_purchase_price, r.cogs_estimated, r.est_profit, r.profit_pct);
      }
      return row;
    });
    await exportToExcel([{ name: 'المنتجات', headers, rows }], `تقرير المنتجات ${fromDate}-${toDate}`);
  };
  ...
  return <ReportShell title="تقرير المنتجات" subtitle={`أداء المنتجات والمبيعات والمشتريات — ${fromDate} → ${toDate}`} onExport={handleExport} isLoading={isLoading} isError={isError} refetch={refetch} reportId="products">
    ...
    {data && (
      <>
        <div className="kpis" style={{ gridTemplateColumns: canViewCost ? 'repeat(4,1fr)' : 'repeat(5,1fr)' }}>
          {canViewCost && <KpiCard variant="teal" icon="ti-building-warehouse" label="قيمة المخزون" value={MONEY(data.summary.total_stock_value)}/>}
          ...
          {canViewCost && <KpiCard variant="green" icon="ti-coins" label="الربح المقدر" value={MONEY(data.summary.total_est_profit)}/>}
          {canViewCost && <KpiCard variant="gold" icon="ti-percentage" label="هامش الربح %" value={`${data.summary.margin_pct}%`}/>}
        ...
        <Card noHeader style={{ padding: 0, marginTop: 16 }}>
          <SimpleTable
            columns={[
              ...
              ...(canViewCost ? [{ key: 'avg_purchase_price', label: 'متوسط سعر الشراء', className: 'num', render: (v: unknown) => FMT(Number(v ?? 0)) }] : []),
              ...
              ...(canViewCost ? [
                { key: 'cogs_estimated', label: 'تكلفة البضاعة', className: 'num', render: (v: unknown, row: { id?: string }) => row.id === '__summary' ? <span style={{ fontWeight: 800 }}>{FMT(v as number)}</span> : FMT(v as number) },
                { key: 'est_profit', label: 'الربح المقدر', className: 'num', render: (v: unknown, row: { id?: string }) => row.id === '__summary' ? <span style={{ fontWeight: 800 }}>{FMT(v as number)}</span> : <span style={{ color: (v as number) >= 0 ? 'var(--em)' : 'var(--red)', fontWeight: 700 }}>{FMT(v as number)}</span> },
                { key: 'profit_pct', label: 'الهامش %', render: (v: unknown, row: { id?: string }) => row.id === '__summary' ? null : <span style={{ color: (v as number) >= 0 ? 'var(--em)' : 'var(--red)' }}>{v as React.ReactNode}%</span> },
              ] : []),
