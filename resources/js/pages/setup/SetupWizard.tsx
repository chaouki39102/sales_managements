...
const SECTORS = ['تجارة التجزئة', 'تجارة الجملة', 'الصناعة والتصنيع', 'الخدمات', 'البناء والأشغال', 'الفلاحة', 'النقل', 'الصحة والصيدلة'];
const BANKS = ['BNA', 'BEA', 'CPA', 'BADR', 'BDL', 'CNEP', 'AGB', 'ABC', 'Société Générale Algérie', 'Al Salam Bank'];
const WILAYAS = [
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 0, marginBottom: 36, position: 'relative' }}>
      ...
      <div style={{
        ...
        background: 'var(--b2)', zIndex: 0,
      }} />
      <div style={{
        position: 'absolute', top: 20, left: '10%', height: 2, zIndex: 1,
        width: `${((step - 1) / (total - 1)) * 80}%`,
        background: 'var(--em)', transition: 'width .5s ease',
      }} />
        ...
        return (
          <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, position: 'relative', zIndex: 2 }}>
            <div style={{
              ...
              background: done ? 'var(--em)' : active ? 'var(--em)' : 'var(--bg2)',
            ...
            <span style={{
              ...
              color: active ? 'var(--em)' : done ? 'var(--t2)' : 'var(--t4)',
            }}>
              {labels[i]}
            </span>
          </div>
...
// ── Nav buttons ───────────────────────────────────
function NavBtns({
  step, total, onPrev, onNext, onFinish, loading, canNext,
}: {
      ...
      <div className="fgrid c3" style={{ gap: 14 }}>
        <Field label="اسم المؤسسة / الشركة" required span={3}>
          <input
            ...
            placeholder="مثال: مؤسسة النور للتجارة العامة"
            autoFocus
          />
        </Field>
        ...
        <Field label="العنوان الكامل" required span={2}>
          <input
            ...
            placeholder="الحي، الشارع، الرمز البريدي"
          />
        </Field>
        ...
        <Field label="البريد الإلكتروني">
          <input
            ...
            placeholder="info@company.dz"
            dir="ltr"
          />
        </Field>

        <Field label="الموقع الإلكتروني" hint="اختياري" span={3}>
          <input
            value={form.website}
            onChange={e => onChange('website', e.target.value)}
            placeholder="www.company.dz"
            dir="ltr"
            style={{ maxWidth: 320 }}
          />
        </Field>
...
// ════════════════════════════════════════════════
// STEP 2 — المعرّفات الجبائية والتجارية
// ════════════════════════════════════════════════
function Step2({ form, onChange }: { form: TaxForm; onChange: (k: keyof TaxForm, v: string) => void }) {
  const fields = [
    { key: 'nif' as const, label: 'NIF — رقم التعريف الجبائي', hint: '15 خانة رقمية', max: 15, pattern: '[0-9]{15}', placeholder: '001234567890123' },
    { key: 'nis' as const, label: 'NIS — الرقم الإحصائي',      hint: 'مركز الإحصاء الوطني', max: 15, pattern: '[0-9]{15}', placeholder: '245103002000012' },
    { key: 'ai'  as const, label: 'AI — رقم المادة الجبائية',  hint: 'مديرية الضرائب الولائية', max: 20, placeholder: '29202400012' },
    { key: 'rc'  as const, label: 'RC — السجل التجاري',        hint: 'المركز الوطني للسجل التجاري CNRC', max: 30, placeholder: '29/00-0012345B05', span: 2 },
  ];
  ...
  return (
    <div>
      ...
      <div style={{
        ...
        borderRadius: 'var(--r2)', padding: '10px 14px', marginBottom: 20,
      }}>
        ...
        <span style={{ fontSize: 12, color: 'var(--t2)', lineHeight: 1.6 }}>
          تأكد من صحة هذه الأرقام — تُستخدم في الفواتير الرسمية وتقارير TVA ولا يمكن تعديلها بسهولة لاحقاً
        </span>
      </div>

      <div className="fgrid c3" style={{ gap: 14 }}>
        {fields.map(f => (
          <Field key={f.key} label={f.label} hint={f.hint} required span={(f as any).span}>
            <input
              value={form[f.key]}
        ...
        <Field label="رقم الحساب البنكي RIB / IBAN" hint="مثال: 00799999000XXXXXXXX00" span={2}>
          <input
            value={form.rib}
            onChange={e => onChange('rib', e.target.value)}
            placeholder="00799999000XXXXXXXX00"
            style={{ fontFamily: 'monospace', letterSpacing: 1 }}
            dir="ltr"
          />
        </Field>
...
// ════════════════════════════════════════════════
// STEP 3 — الشعار والهوية
// ════════════════════════════════════════════════
function Step3({
  logo, onLogo, companyName,
}: {
  logo: File | null;
  ...
  return (
    <div>
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ fontSize: 13, color: 'var(--t3)' }}>يظهر الشعار على الفواتير والوثائق الرسمية — يمكن تعديله لاحقاً</p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
        ...
        <div>
          <div
            ...
            style={{
              border: `2px dashed ${dragging ? 'var(--em)' : preview ? 'var(--em)' : 'var(--b3)'}`,
              borderRadius: 'var(--r3)', padding: 32, textAlign: 'center', cursor: 'pointer',
              background: dragging ? 'var(--emb)' : 'var(--bg3)',
          ...
          >
            {preview ? (
              ...
              <>
                ...
                <div style={{ fontSize: 11, color: 'var(--t4)' }}>PNG, SVG, JPG — حجم موصى 400×400 بكسل</div>
              </>
            )}
          </div>
          <input
          ...
          {preview && (
            <button
              className="btn"
              style={{ marginTop: 8, width: '100%', color: 'var(--red)' }}
              onClick={() => { onLogo(null); setPreview(null); }}
            >
              <i className="ti ti-trash" /> إزالة الشعار
            </button>
          )}
          <p style={{ fontSize: 11, color: 'var(--t4)', textAlign: 'center', marginTop: 10 }}>
            يمكنك تخطي هذه الخطوة وإضافة الشعار لاحقاً من الإعدادات
          </p>
        </div>

        {/* Preview invoice header */}
        <div>
          <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--t3)', marginBottom: 10, textTransform: 'uppercase', letterSpacing: 1 }}>
            معاينة رأس الفاتورة
          </div>
          <div style={{
            background: '#fff', border: '1px solid #e2e8f0', borderRadius: 10, padding: 20,
            fontFamily: 'serif', boxShadow: '0 2px 12px rgba(0,0,0,0.06)',
          }}>
            <div style={{ display: 'flex', gap: 14, alignItems: 'center', borderBottom: '2px solid #0a9268', paddingBottom: 14, marginBottom: 12 }}>
              {preview && (
                <img src={preview} alt="" style={{ height: 48, width: 48, objectFit: 'contain', borderRadius: 6 }} />
              )}
              <div>
                <div style={{ fontWeight: 900, fontSize: 15, color: '#0a8a5c', lineHeight: 1.2 }}>
            ...
            <div style={{ fontSize: 10, color: '#64748b', lineHeight: 1.8, fontFamily: 'monospace' }}>
              ...
              <div style={{ fontFamily: 'sans-serif', color: '#475569' }}>ورقلة، الجزائر — 030 71 23 45</div>
            </div>
            <div style={{ marginTop: 14, borderTop: '1px dashed #e2e8f0', paddingTop: 10 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, fontWeight: 700, color: '#0f172a' }}>
                <span>فاتورة بيع</span>
                <span style={{ fontFamily: 'monospace', color: '#0a9268' }}>FAC-2025-000001</span>
              </div>
            </div>
          </div>
        </div>
  ...
  return (
    <div>
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ fontSize: 13, color: 'var(--t3)' }}>يتم تعيينها كسنة مالية نشطة تلقائياً — لا يمكن حذف الأولى</p>
      </div>
      ...
      <div style={{
        ...
        <div style={{ fontSize: 12, color: 'var(--t2)', lineHeight: 1.7 }}>
          ...
          يمكنك تغيير التواريخ إذا كانت مؤسستك تبدأ السنة المالية في تاريخ مختلف.
        </div>
      </div>
      ...
      <div className="fgrid c3" style={{ gap: 14 }}>
        <Field label="اسم السنة المالية" required span={3}>
          <input
            value={form.name}
            onChange={e => onChange('name', e.target.value)}
            placeholder={`مثال: السنة المالية ${currentYear}`}
            style={{ maxWidth: 320 }}
          />
        </Field>
        ...
        <div className="fg">
          ...
          <div style={{
            ...
            borderRadius: 'var(--r1)', fontSize: 13, color: 'var(--t3)',
          }}>
      ...
      {form.name && form.start_date && form.end_date && (
        <div style={{
          ...
          borderRadius: 'var(--r2)', display: 'flex', alignItems: 'center', gap: 12,
        }}>
          ...
          <div style={{ fontSize: 13, color: 'var(--t1)' }}>
            ...
            <strong>{new Date(form.end_date).toLocaleDateString('ar-DZ')}</strong> كسنة مالية نشطة
          </div>
        </div>
      )}
    </div>
...
export default function SetupWizard({ onComplete }: { onComplete: () => void }) {
  ...
  const handleFinish = async () => {
    ...
    try {
      // 1. حفظ إعدادات الشركة في settings
      ...
      await Promise.all(
        settingsPayload
          .filter(s => s.value)
          .map(s => apiClient.post('/settings', s))
      );
      ...
      if (logo) {
        ...
        }).catch(() => null); // لا يوقف الإعداد إذا فشل رفع الشعار
      }
      ...
      // 4. تحديث الـ flag وتوجيه للداشبورد
      onComplete();
      navigate('/dashboard', { replace: true });

    } catch (err: unknown) {
      const msg = (err as { response?: { data?: { message?: string } } })?.response?.data?.message;
      setError(msg || 'حدث خطأ أثناء الحفظ. تحقق من الاتصال وأعد المحاولة.');
    } finally {
      setSaving(false);
    }
  };
      ...
      <div style={{ width: '100%', maxWidth: 780 }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 40 }}>
          <div style={{
            ...
            background: 'var(--emb)', border: '1px solid var(--embo)',
            borderRadius: 'var(--r4)', padding: '6px 18px', marginBottom: 16,
          }}>
          ...
          <p style={{ fontSize: 14, color: 'var(--t3)', maxWidth: 460, margin: '0 auto' }}>
            سنقوم معاً بإعداد النظام في دقائق قليلة — يمكنك تعديل أي معلومة لاحقاً من الإعدادات
          </p>
        </div>

        {/* Card */}
        <div style={{
          background: 'var(--bg2)', borderRadius: 'var(--r4)',
          border: '1px solid var(--b2)', padding: '32px 36px',
          ...
          {error && (
            <div style={{
              ...
              padding: '10px 14px', background: 'var(--redb)',
              border: '1px solid var(--redbo)', borderRadius: 'var(--r2)',
            }}>
