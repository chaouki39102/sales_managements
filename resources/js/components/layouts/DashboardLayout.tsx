...
// الإصدار النهائي – مع دعم الإعداد الأولي للسنة المالية
// ════════════════════════════════════════════════
...
// ─── البيانات الوصفية للصفحات (للـ Topbar) ───────────────
...
export default function DashboardLayout() {
  ...
  // ═══════════════════════════════════════════════
  // فحص / إنشاء السنة المالية
  // ═══════════════════════════════════════════════
  const { company } = useParams<{ company: string }>();
  const [fiscalState, setFiscalState] = useState<'loading' | 'noYear' | 'ready'>('loading');
  const [yearName, setYearName] = useState('');
  const [creating, setCreating] = useState(false);

  useEffect(() => {
    if (!company) return;
    apiClient.get(`/${company}/fiscal-years/current`)
      .then(res => {
        const year = res.data?.data;
        setFiscalState(year ? 'ready' : 'noYear');
      })
      .catch(() => setFiscalState('noYear'));
  }, [company]);
  ...
  const handleCreateYear = async () => {
    ...
    setCreating(true);
    try {
      const start = `${yearName}-01-01`;
      const end   = `${yearName}-12-31`;
      await apiClient.post(`/${company}/fiscal-years`, {
        name: yearName,
        start_date: start,
        end_date: end,
      });
      ...
      alert(e?.response?.data?.message || 'فشل إنشاء السنة المالية');
    } finally {
      setCreating(false);
    }
  };
  ...
  // ─── بيانات Topbar ───────────────────────────────────
  const currentPath = location.pathname.split('/').slice(2).join('/') || 'dashboard';
  ...
  return (
    <>
      ...
      <main id="main">
        ...
        {/* ════════════ منطقة المحتوى – مع تحقق السنة المالية ════════════ */}
        <div style={{ flex: 1 }}>
          {fiscalState === 'loading' && (
            <div style={{ display:'flex', alignItems:'center', justifyContent:'center', height:'100%' }}>
              <span className="ic ic-lg" style={{ color:'var(--em)' }}>
                <i className="ti ti-loader" style={{ animation:'spin 1s linear infinite' }} />
              </span>
              <span style={{ marginRight: 8 }}>تحميل السنة المالية...</span>
            </div>
          )}
          ...
          {fiscalState === 'noYear' && (
            <div style={{ display:'flex', alignItems:'center', justifyContent:'center', height:'100%' }}>
              <div style={{
                ...
                <h2 style={{ marginBottom: 12, fontSize: 20 }}>لا توجد سنة مالية نشطة</h2>
                ...
                >
                  {creating ? 'جارٍ الإنشاء...' : 'إنشاء السنة المالية'}
                </button>
              </div>
            </div>
          )}
