// ════════════════════════════════════════════════════════════════════════════
// DataTable/hooks.ts  —  v10.0
//
// ✅ كل hooks v9 محفوظة بالكامل
//
// 🆕 useRowGrouping      — تجميع الصفوف + collapse/expand للمجموعات
// 🆕 useColumnPinning    — تثبيت/فك تثبيت الأعمدة ديناميكياً
// 🆕 useKeyboardNav      — تنقل Tab/Arrows/F2/Escape بين الخلايا
// 🆕 useBatchEdit        — تجميع التعديلات + Undo (Ctrl+Z) + Redo (Ctrl+Y)
// 🆕 useCellValidation   — التحقق من قيمة الخلية قبل الحفظ
// ════════════════════════════════════════════════════════════════════════════
...
export function useMultiSort(
  ...
  return { sorts, toggleSort, clearSort, legacySortState };
}

// ─── 🆕 useRowGrouping ───────────────────────────────────────────────────────
//
// يجمّع الصفوف حسب قيمة عمود ويدير collapse/expand لكل مجموعة.
...
// ─── 🆕 useColumnPinning ─────────────────────────────────────────────────────
//
// يدير تثبيت/فك تثبيت الأعمدة ديناميكياً.
// يدمج static sticky من Column definition مع الـ pins الديناميكية.
...
// ─── 🆕 useKeyboardNav ───────────────────────────────────────────────────────
//
// تنقل لوحة المفاتيح بين خلايا الجدول.
// Arrow keys للتنقل، F2/Enter للتعديل، Escape للإلغاء، Tab للتنقل أفقياً.

export function useKeyboardNav({
  enabled,
  rowCount,
  colCount,
  onActivate,        // عند الوصول لخلية (للـ scroll)
  onStartEdit,       // F2 أو Enter
}: {
  enabled:    boolean;
  rowCount:   number;
  colCount:   number;
  ...
  const handleKeyDown = useCallback((e: React.KeyboardEvent) => {
    ...
    // في وضع التعديل: فقط Escape يُغلق
    if (isEditing) {
    ...
    switch (e.key) {
      ...
      case 'ArrowRight': e.preventDefault(); move(0, -1); break; // RTL
      case 'ArrowLeft':  e.preventDefault(); move(0, +1); break; // RTL
      case 'Tab':
        e.preventDefault();
...
// ─── 🆕 useBatchEdit ─────────────────────────────────────────────────────────
//
// يجمع تعديلات الخلايا في stack.
// Ctrl+Z = Undo, Ctrl+Y / Ctrl+Shift+Z = Redo.
// onBatchSave يُستدعى عند الضغط على "حفظ التعديلات".
...
export function useBatchEdit({
  ...
  const undo = useCallback(() => {
    setState(prev => {
      ...
      last.forEach(e => {
        ...
        if (e.oldValue === '') {
          delete rowPending[e.colKey];
        } else {
          rowPending[e.colKey] = e.oldValue;
        }
  ...
  const save = useCallback(() => {
    ...
    Object.entries(state.pending).forEach(([rowKey, cols]) => {
      Object.entries(cols).forEach(([colKey, newValue]) => {
        // نجد old value من آخر history entry
  ...
  const pendingCount = useMemo(
    () => Object.values(state.pending).reduce((acc, cols) => acc + Object.keys(cols).length, 0),
    [state.pending],
  );
...
// ─── 🆕 useCellValidation ────────────────────────────────────────────────────
//
// يتحقق من قيمة الخلية حسب قواعد Column.validation.
// يُعيد: { errors, validate, clearError }
...
export function useCellValidation() {
  ...
  const validate = useCallback((
    ...
    if (rule.required && !value.trim()) {
      msg = rule.messages?.required ?? 'هذا الحقل مطلوب';
    } else if (rule.minLength && value.length < rule.minLength) {
      msg = rule.messages?.minLength ?? `الحد الأدنى ${rule.minLength} أحرف`;
    } else if (rule.maxLength && value.length > rule.maxLength) {
      msg = rule.messages?.maxLength ?? `الحد الأقصى ${rule.maxLength} أحرف`;
    } else if (rule.min !== undefined && parseFloat(value) < rule.min) {
      msg = rule.messages?.min ?? `القيمة لا تقل عن ${rule.min}`;
    } else if (rule.max !== undefined && parseFloat(value) > rule.max) {
      msg = rule.messages?.max ?? `القيمة لا تزيد عن ${rule.max}`;
    } else if (rule.pattern && !rule.pattern.test(value)) {
      msg = rule.messages?.pattern ?? 'القيمة غير صحيحة';
    } else if (rule.custom) {
      msg = rule.custom(value, row);
    }

    if (msg) {
      setErrors(prev => ({ ...prev, [cellId]: msg as string }));
      return false;
    }
