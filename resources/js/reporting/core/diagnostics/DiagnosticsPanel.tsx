          ...
          {latest && (
            <>
              ...
              {latest.warnings.length > 0 && (
                <span style={{ color: '#ffa726' }}>⚠ {latest.warnings.length}</span>
              )}
              {latest.errors.length > 0 && (
                <span style={{ color: '#ef5350' }}>✕ {latest.errors.length}</span>
              )}
            </>
          )}
              ...
              {latest.warnings.length > 0 && (
                <div style={{ marginBottom: 4 }}>
                  ...
                  {latest.warnings.map((w, i) => (
                    <div key={i} style={{ fontSize: 9, color: '#ccc', padding: '1px 4px' }}>⚠ {w}</div>
                  ))}
                </div>
              )}
              ...
              {latest.errors.length > 0 && (
                <div>
                  ...
                  {latest.errors.map((e, i) => (
                    <div key={i} style={{ fontSize: 9, color: '#ef9a9a', padding: '1px 4px' }}>✕ {e}</div>
                  ))}
                </div>
              )}
            </div>
