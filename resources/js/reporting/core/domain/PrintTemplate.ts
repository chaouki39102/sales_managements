// ════════════════════════════════════════════════════════════════════════════
// reporting/core/domain/PrintTemplate.ts
//
// The canonical PrintTemplate type for the framework.
// This is Layer 1 — zero dependencies on React, Axios, or any runtime.
//
// Relationship to existing code:
//   - pages/settings/print-settings/types.ts exports PrintTemplate
//   - That type is an ALIAS of this one (pointing here)
//   - All existing code continues to work unchanged
//
// When to add fields here:
//   - When a new printable property is needed
//   - After verifying: DB column, API serialization, UI control, Preview,
//     Print, Import/Export (per the Phase 0 property audit checklist)
// ════════════════════════════════════════════════════════════════════════════

// ─── Re-export existing enums (single source of truth) ────────────────────────
//
// These are already defined in pages/settings/print-settings/types.ts.
// During Phase 0 we keep them there and re-export from here so both
// paths work. In Phase 1, the canonical definition moves here.
...
// ─── PrintTemplate ────────────────────────────────────────────────────────────
//
// PROPERTY AUDIT STATUS (Phase 0 — to be completed):
// Format:  ✅ fully implemented | ⚠️ partial | ❌ not implemented | 🗑 deprecated
//
// Legend columns: DB | API | Type | UI | Preview | Print
// Each property below has an audit comment when status is non-trivial.

export interface PrintTemplate {
  // ── Identity ────────────────────────────────────────────────────────────
  id:            number | null;     // ✅ DB | ✅ API | ✅ Type | ✅ UI | ✅ | ✅
  name:          string;            // ✅ DB | ✅ API | ✅ Type | ✅ UI | — | —
  doc_type_code: DocTypeCode;       // ✅ DB | ✅ API | ✅ Type | ✅ UI | — | —
  paper_size:    PaperSize;         // ✅ DB | ✅ API | ✅ Type | ✅ UI | ✅ | ✅
  is_default:    boolean;           // ✅ DB | ✅ API | ✅ Type | ✅ UI | — | —
  is_active:     boolean;           // ✅ DB | ✅ API | ✅ Type | ✅ UI | — | —
  created_at?:   string;
  updated_at?:   string;
  ...
  // ── Company overrides (template-level overrides API company data) ─────────
  // ⚠️ Preview: used. Print: used. UI: used. API serialization: in config blob.
  // DB: stored inside JSON config, not as flat columns. OK for now.
  ...
  // ⚠️ Below three: UI toggle present, but A4Preview has no data source yet.
  // Phase 2: DocumentDataBuilder.fromApiDocument provides this data.
  show_delivery_address: boolean;  // ⚠️ No data yet — Phase 2
  show_session:          boolean;  // ⚠️ No data yet — Phase 2
  show_payment_term:     boolean;  // ⚠️ No data yet — Phase 2
