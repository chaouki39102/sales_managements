...
export const DesignerToolbar: React.FC = () => {
  ...
  return (
    <div style={{
      ...
      {/* Undo/Redo */}
      <button style={{ ...btn, opacity: canUndo ? 1 : 0.4 }} onClick={undo} disabled={!canUndo} title="Undo (Ctrl+Z)">↩</button>
      <button style={{ ...btn, opacity: canRedo ? 1 : 0.4 }} onClick={redo} disabled={!canRedo} title="Redo (Ctrl+Y)">↪</button>
      ...
      <button style={btn} onClick={() => handleAddElement('image', 'Image')} title="Add image">🖼</button>
      <button style={btn} onClick={() => handleAddElement('barcode', 'Barcode')} title="Add barcode">⊞</button>
      <button style={btn} onClick={() => handleAddElement('line', 'Line')} title="Add line">━</button>
      <button style={btn} onClick={() => handleAddElement('rectangle', 'Rectangle')} title="Add rectangle">▬</button>

      <span style={separator} />

      {/* Clipboard */}
      <button style={{ ...btn, opacity: hasSelection ? 1 : 0.4 }} onClick={copySelected} disabled={!hasSelection} title="Copy (Ctrl+C)">📋</button>
      <button style={btn} onClick={pasteClipboard} title="Paste (Ctrl+V)">📄</button>
      <button style={{ ...btn, opacity: hasSelection ? 1 : 0.4 }} onClick={duplicateSelected} disabled={!hasSelection} title="Duplicate (Ctrl+D)">⧉</button>
      <button style={{ ...btn, opacity: hasSelection ? 1 : 0.4 }} onClick={deleteSelected} disabled={!hasSelection} title="Delete (Del)">✕</button>

      <span style={separator} />

      {/* Layer */}
      <button style={{ ...btn, opacity: hasSelection ? 1 : 0.4 }} onClick={() => selectedIds.forEach(id => bringForward(id))} disabled={!hasSelection} title="Bring forward">↑</button>
      <button style={{ ...btn, opacity: hasSelection ? 1 : 0.4 }} onClick={() => selectedIds.forEach(id => sendBackward(id))} disabled={!hasSelection} title="Send backward">↓</button>
      ...
      <button style={snapToGrid ? activeBtn : btn} onClick={toggleSnapToGrid} title="Snap to grid">⚡</button>
      <button style={showRulers ? activeBtn : btn} onClick={toggleRulers} title="Toggle rulers">📏</button>
      <button style={showSafeArea ? activeBtn : btn} onClick={toggleSafeArea} title="Toggle safe area">▭</button>
