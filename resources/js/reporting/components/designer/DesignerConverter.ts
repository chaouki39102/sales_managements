import type { PrintTemplate, AlignOption } from '../../core/domain/PrintTemplate';
...
function buildHeaderSection(tpl: PrintTemplate): SectionElements {
  ...
  if (tpl.show_logo) {
    ...
    children.push({
      ...
      label: 'الشعار', sectionType: 'header',
  ...
  if (tpl.show_company_name) {
    ...
    children.push({
      ...
      label: 'اسم الشركة', sectionType: 'header',
  ...
  if (tpl.show_address) infoItems.push('العنوان');
  if (tpl.show_phone) infoItems.push('الهاتف');
  if (tpl.show_tax_id) infoItems.push('رقم الجباية');
  if (tpl.show_rc) infoItems.push('ر.س.ت');
  if (tpl.show_nis) infoItems.push('ن.إ.ض');
  if (tpl.show_ice) infoItems.push('I.C.E');
  if (tpl.show_article) infoItems.push('المادة');
  ...
  if (infoItems.length > 0 || tpl.override_address || tpl.override_phone || tpl.override_nif) {
    children.push({
      ...
      label: 'معلومات الشركة', sectionType: 'header',
  ...
  if (tpl.header_custom_text) {
    children.push({
      ...
      label: 'نص مخصص', sectionType: 'header',
  ...
  children.push({
    ...
    label: 'فاصل الرأس', sectionType: 'header',
...
function buildDocInfoSection(tpl: PrintTemplate): SectionElements {
  ...
  const rows: { label: string; key: string; visible: boolean }[] = [
    { label: 'عنوان المستند', key: 'title_text', visible: true },
    { label: 'رقم المستند', key: 'show_doc_number', visible: tpl.show_doc_number },
    { label: 'التاريخ', key: 'show_date', visible: tpl.show_date },
    { label: 'الوقت', key: 'show_time', visible: tpl.show_time },
    { label: 'تاريخ الاستحقاق', key: 'show_due_date', visible: tpl.show_due_date },
    { label: 'الكاشير', key: 'show_cashier', visible: tpl.show_cashier },
    { label: 'العميل', key: 'show_client', visible: tpl.show_client },
    { label: 'رقم جباية العميل', key: 'show_client_nif', visible: tpl.show_client_nif },
    { label: 'هاتف العميل', key: 'show_client_phone', visible: tpl.show_client_phone },
    { label: 'عنوان العميل', key: 'show_client_address', visible: tpl.show_client_address },
    { label: 'عنوان التسليم', key: 'show_delivery_address', visible: tpl.show_delivery_address },
    { label: 'الجلسة', key: 'show_session', visible: tpl.show_session },
    { label: 'شروط الدفع', key: 'show_payment_term', visible: tpl.show_payment_term },
  ];

  const titleSize = tpl.title_size || 13;
  children.push({
    id: nextId(), type: DESIGNER_ELEMENT_TYPES.DOC_TITLE,
    label: 'عنوان المستند', sectionType: 'doc-info',
  ...
  children.push({
    ...
    label: 'معلومات المستند', sectionType: 'doc-info',
  ...
  if (tpl.show_bank_details) {
    children.push({
      ...
      label: 'معلومات بنكية', sectionType: 'doc-info',
      x: 8, y, width: 734, height: 30,
      rotation: 0, locked: false, visible: true,
      parentId: id, children: [],
      styles: { fontSize: '9px' },
  ...
  children.push({
    id: nextId(), type: DESIGNER_ELEMENT_TYPES.SEPARATOR,
    label: 'فاصل المستند', sectionType: 'doc-info',
    x: 8, y, width: 734, height: 2,
    rotation: 0, locked: false, visible: true,
    parentId: id, children: [],
    styles: {},
...
function buildItemsSection(tpl: PrintTemplate): SectionElements {
  ...
  if (tpl.show_col_header) {
    children.push({
      id: nextId(), type: DESIGNER_ELEMENT_TYPES.SEPARATOR,
      label: 'رأس الجدول', sectionType: 'items',
  ...
  children.push({
    ...
    label: 'جدول الأصناف', sectionType: 'items',
...
function buildTotalsSection(tpl: PrintTemplate): SectionElements {
  ...
  children.push({
    ...
    label: 'الإجماليات', sectionType: 'totals',
...
function buildPaymentsSection(tpl: PrintTemplate): SectionElements {
  ...
  children.push({
    ...
    label: 'المدفوعات', sectionType: 'payments',
...
function buildFooterSection(tpl: PrintTemplate): SectionElements {
  ...
  children.push({
    id: nextId(), type: DESIGNER_ELEMENT_TYPES.SEPARATOR,
    label: 'فاصل التذييل', sectionType: 'footer',
    x: 8, y, width: 734, height: 2,
    rotation: 0, locked: false, visible: true,
    parentId: id, children: [],
    styles: {},
  ...
  if (tpl.footer_line1 || tpl.footer_line2 || tpl.footer_line3) {
    children.push({
      ...
      label: 'نص التذييل', sectionType: 'footer',
  ...
  if (tpl.show_returns_policy) {
    children.push({
      ...
      label: 'سياسة الإرجاع', sectionType: 'footer',
  ...
  if (tpl.show_barcode) {
    children.push({
      ...
      label: 'باركود', sectionType: 'footer',
  ...
  if (tpl.show_cashier_signature || tpl.show_client_signature || tpl.show_stamp) {
    children.push({
      ...
      label: 'التوقيعات', sectionType: 'footer',
      x: 8, y, width: 734, height: 30,
      rotation: 0, locked: false, visible: true,
      parentId: id, children: [],
      styles: {},
