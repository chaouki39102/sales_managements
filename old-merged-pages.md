...
function FiscalYearModal({
  ...
  const handleCreateYear = async () => {
    ...
    try {
      // ✅ المسار الصحيح: /{slug}/fiscal-years
      await client.post(`/${company.slug}/fiscal-years`, {
        name,
...
function AdminModal({
  ...
  const handleSaveCompany = async () => {
    ...
    setSaving(true);
    try {
      await client.put(`/companies/${editTarget.slug}`, form);
  ...
  const handleVerify = async (co: Company) => {
    try {
      ...
      await client.post(`/admin/companies/${co.slug}/${endpoint}`);
      setCompanies(prev => prev.map(c =>
  ...
  const handleChangePlan = async (co: Company, plan: typeof PLANS[number]) => {
    try {
      await client.patch(`/admin/companies/${co.slug}/plan`, { plan });
      setCompanies(prev => prev.map(c =>
        c.id === co.id ? { ...c, plan } : c
      ));
  ...
  const handleSaveNotes = async () => {
    ...
    setSaving(true);
    try {
      await client.patch(`/admin/companies/${editTarget.slug}/notes`, { notes: form.notes });
      setCompanies(prev => prev.map(c =>
...
export default function OnboardingPage() {
  ...
  const handleCompanyClick = async (company: Company) => {
    ...
    setError(null);
    try {
      await client.post('/companies/switch', { company_id: company.id });
