...
class PartyBalanceService
{
    ...
    public function getBalanceAt(int $partyId, string $date): array
    {
        ...
        // 2. Sale/purchase documents
        // ✅ إصلاح: dbo.code بدل dbo.name (name قد يكون عربي أو مترجم)
        // ✅ إصلاح: is_locked هو العمود الصحيح (مؤكد من CommercialDocument::$fillable)
        $documentsBalance = DB::table('commercial_documents as cd')
            ...
            ->selectRaw("
                COALESCE(SUM(CASE WHEN dbo.code = 'sale'     THEN cd.net_to_pay ELSE 0 END), 0)
                -
                COALESCE(SUM(CASE WHEN dbo.code = 'purchase' THEN cd.net_to_pay ELSE 0 END), 0)
                as balance
            ")
            ->value('balance') ?? 0;
     ...
     * ✅ إصلاح: JOIN مع party_types بدل PartyType::find() داخل foreach (N+1)
     */
    public function getAllBalancesAt(string $date, ?int $partyTypeId = null, ?string $search = null): array
    {
        $companyId = $this->companyContext->get();

        // ✅ JOIN مع party_types في نفس الاستعلام — يلغي N+1
        ...
        $parties = $partyQuery
            ->select('p.id', 'p.name', 'p.party_type_id', 'pt.name as party_type_name', 'pt.code as party_type_code')
            ->get();
        ...
        foreach ($parties as $party) {
            ...
            $balance['party'] = [
                ...
                'name'       => $party->name,
                'party_type' => [
                    'name' => $party->party_type_name,
                    'code' => $party->party_type_code,
                ],
            ];
            $balances[] = $balance;
        }
