...
- **The globally-mounted `CommercialDocumentModal` (App.tsx `DocumentQuickCreateProvider`) keeps its whole body in the DOM when closed** — the overlay is hidden only via `opacity: 0` + `pointerEvents: none` (index.tsx:526-539), NOT `if (!open) return null`. So every page carries a hidden second `DocumentLinesSection` (hence a hidden duplicate camera button). **Playwright's visibility check ignores opacity** — a strict locator `button[title="…"]` resolves to 2 elements on any documents page. Any spec hitting the documents form must scope to the real page: `page.locator('#main button[title="مسح الباركود بالكاميرا"]')`. Similarly scope modal-overlay clicks to `div[style*="z-index: 99999"]` when other «إلغاء» buttons exist on the page.
