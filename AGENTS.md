...
- **Commit and push AFTER ANY TASK** (user directive, Sep 6 2026): any completed work — task, group, batch, or fix — must be committed and pushed to the remote before moving on. Do not leave working-tree state behind at session end. Exception: preference/machine-local changes intentionally not committed (e.g. `php.ini` opcache, `share-public-order.config.ps1` gitignored) — say so explicitly when skipping.
...
- **NEVER use `window.confirm()` or `window.alert()`** — always use the project's custom confirm modal: `useConfirm` hook + `ConfirmDialog` component. Path: `resources/js/hooks/useConfirm.ts` (hook) + `resources/js/components/ui/ConfirmDialog.tsx` (component). Usage: `const { confirm, confirmDialogProps } = useConfirm()` → call `await confirm('message', options?)` → render `<ConfirmDialog {...confirmDialogProps} />` at the bottom of the component's JSX. Native browser dialogs are blocked by UX design and must not appear anywhere in the app.
...
- **Full code review COMPLETE** (Aug 23, 2026) — `CODE_REVIEW_TODO.md` (project root) holds the full ledger: all 8 sections reviewed, 81 findings (46 fixed + 35 observations/verified), one commit per section (`ea4eb22` … `06ef933`). The Findings table there is the SSOT for what was changed vs. deliberately left as-is — consult it before "fixing" anything an observation row already ruled on.
- **Windows print helper (`WindowsPrinterService`) must be compiled once from CLI**: the native helper (`storage/app/tools/posdz_print_helper.exe`, built by inbox `csc.exe` C#5) compiles fine from any CLI php, but AV heuristics block csc when spawned from the web-server chain (`CS0016 Accès refusé` writing the output tmp) — `ensureHelper()` logs `[WinPrint] compile failed` and falls back to the last good exe (call-contract is stable). If the exe is missing on a fresh clone, trigger one CLI call (tinker smoke or first POS print from a terminal-run artisan serve) to build it. Also: **OneNote printers fail under web** (`Win32Exception Accès refusé` — their driver needs an interactive desktop for the export UI); real spooler-only printers (Canon MF3010 etc.) print fine via HTTP. All spawned processes use explicit temp CWDs — never remove that, or Windows tools litter `%SystemDrive%\...` cache dirs into the repo.
...
**Phase 86 follow-up — lines still vanished on short laptops (15.6", innerHeight<820)**: even with the Phase 86 narrow/compact stacking, the wide layout (1366px is > 1180, so NARROW is false) stacked everything in the single flex column where the lines wrapper was `flex:1; minHeight:0` and the aux+totals siblings were `flexShrink:0`. On a short viewport the expanded header band + chain/credit/insights/attachments + totals consumed the whole height, so the lines (`flex:1`) collapsed to ~0 — only collapsing the HeaderBand (releasing height) made them reappear. The user's priority: **small 15.6" screens FIRST**, and "the page must not scroll; only the products/lines grid scrolls internally". User rejected sidebar/moving-panels approaches — the chosen fix is a **min-height guard on the lines grid**. Commit `30ea8bc`.
- **Lines wrapper** (`CommercialDocumentPage.tsx`): `flex:1` → `flex:'1 1 auto'` + `minHeight:'min(45vh, 460px)'` — the grid always keeps ~45% of the viewport and can never be crushed to zero.
- **Aux panels + totals merged into ONE bounded region**: previously two `flexShrink:0` blocks (chain/credit/insights/attachments, then `DocumentTotalsSection`). Now a single region `flex:'0 1 auto'; minHeight:0; overflowY:'auto'; display:flex; flexDirection:column; gap` — it shrinks (it can no longer push lines off-screen) and scrolls internally when its content exceeds the remaining space. The page itself still never scrolls; only the lines grid and this aux region scroll internally.
- **HeaderBand auto-collapse**: `bandCollapsed` now defaults to **true on screens with `innerHeight < 820`** (instead of always false), so the tall field band is a thin 36px strip by default on 15.6" laptops — directly mirroring the user's discovery that collapsing the band frees the lines. Any saved manual toggle (`doc_band_collapsed_${docCode}`) is still honored and overrides the default.
**Rules added**: (1) in a non-scrolling `height:100vh` flex column, any fixed-height (`flexShrink:0`) sibling of a `flex:1` grid will crush that grid to zero on short viewports — siblings that can be tall must be `flex:0 1 auto` + `overflowY:auto` (bounded, internally scrollable) so they yield to the primary grid. (2) When the user says "lines must always be visible", enforce it with an explicit `minHeight` on the grid wrapper AND default-collapse the tallest competing band on short screens — do not rely on flex alone. (3) Default-collapse logic must never override a stored manual preference (localStorage wins).
...
**Phase 84 follow-up ("لا يمكن اضافة الاسطر بلوحة المفاتيح فقط" — the REAL blocker was the product picker)**: the qty/price focus work was necessary but NOT sufficient — the flow died at product selection. `ProductSearch` renders a **button trigger + portal dropdown**, and (1) after picking with Enter in the dropdown, focus dropped to `<body>` (portal unmounts) — dead end; (2) the section Enter-walk's `FOCUSABLE` selector (`input/select/textarea`) excludes buttons, so the trigger was unreachable by the walk; (3) Alt+N/new-row focus landed on qty of a PRODUCT-LESS row — useless until a product is picked by mouse. Fixes: `ProductSearch` gained `triggerId` (→ button id + `data-has-product="0|1"` marker) and `afterSelectFocusId` (choose() now rAF-refocuses that cell — the row's qty — and selects it); Escape in the dropdown `stopPropagation`s so the topbar's window-level Escape back-nav (`DocumentTopbar.tsx`) never closes the editor while the picker is open; new-row focus chains are now `['product','qty','total_qty']` (Alt+N, إضافة سطر, generic last-row add polls `doc-line-{newIdx}-product` first); the price→next-row jump targets the next row's **product trigger when `data-has-product="0"`**, else qty. Complete mouseless loop: Alt+N → Enter opens picker → type → ↑/↓ → Enter picks → qty auto-focused → qty ↵ price ↵ next row's picker… **Rules**: any focus-chain into a fresh row must start at its PRODUCT picker when the row is empty (qty on an empty row is a dead end); a combobox whose dropdown is portaled to body must hand focus back itself (`afterSelectFocusId`) because unmounting the portal drops focus to body; `data-has-product` on the trigger lets DOM-level navigation decide picker-vs-qty without reading React state; dropdown Escape must stopPropagation or window-level back-nav fires.
